...
Страница `specialist-booking-calendar` (`CalendarPage.vue`) живёт отдельным пунктом в меню продаж
(`activity_main_menu`, без родителя), а не в настройках — это рабочий экран менеджера. Под «Заказы»
не вкладываем: CRM добавляет страницу модуля в ядерный пункт по наличию подпунктов в конфигурации,
а «Список заказов» и «Канбан» видны только с включённым канбаном. Без него «Заказы» превращались
в группу с одним календарём, и список заказов из меню пропадал. Неделя с
